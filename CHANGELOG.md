...
**[Approximate-Punchup] the `approximate` surface is finished: all five families reach DecL, every fit renders as DecL, the severity keyword mirrors the input, and the object output is parser-born.** The DecL `approximate` keyword and the constructor accept `norm | lognorm | gamma` alongside `sgamma | slognorm` (the unshifted three match mean and cv only; `norm` targets zero skew). The `norm` DecL fragment now spells `scale * norm + loc` (the old `@` / `#` form was unparseable, a real bug). A reflected (left-skew) fit renders through the ordinary reflection syntax `loc - scale * name shape`, so `output='sev_decl'` / `'agg_decl'` and the object mode serve every fit; only `output='scipy'` still raises for a reflected fit. **Results move**: the emitted severity type mirrors the source's signedness on every path, the DecL keyword included, so a plain `sev` input whose fit reaches below zero (a normal, or a reflected fit) clamps to an atom at 0 and the matched moments drift by the clamp mass, reported in `validation_df`; an `ssev` input stays exact. `output='agg'` is the documented object-mode spelling and returns `build(agg_decl)`, so the object carries a `program`, round-trips, and lands in the underwriter's knowledge. `approximate_from_mcvsk` gains `signed_input=` and its `agg_str` argument now excludes the severity keyword. Plan: `dev/plan-approximate-punchup.md`, first of two bumps.

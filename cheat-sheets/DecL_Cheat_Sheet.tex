...
\medskip\it Name of the compound \\
Access from knowledge as \texttt{agg.NAME}. \\
Names must match regex \\
\rm \texttt{r'[a-zA-Z][\textbackslash.\_:~a-zA-Z0-9\textbackslash-]*'}.
...
\medskip\it Occurrence limits applied to ground-up severity, unlimited reinstatements,
losses conditional on attaching layer by default.
...
\texttt{LAYER=SHARE so LAYER xs ATTACH | PARTICIPATION po LAYER xs ATTACH}
\texttt{LAYERS=LAYER1 and LAYER2 and ...}

\medskip\it Amount share of (\texttt{so}); participation (placed) $0\le po\le 1$ part of (\texttt{po})
...
\texttt{MIXING DIST=gamma|delaporte|ig|sichel|beta}
...
\medskip\it Add hints for updating \\
...
{\bf Warning:} \it there is no unary minus, minus binds to the number: $-4^2=(-4)^2=16$.

...
> **Status: CLOSED, moved to `dev/done/` 2026-08-12.** The 1.0 scope landed: the three passes (inventory a197, schema a198, conversions culminating in the eight app emitters by a244), and the first-class `.plot()` methods draw through `plot_chartdoc(build_chart_doc(...))`. What remains open was post-1.0 by this plan's own charter (conversion of charts the app does not use, full matplotlib convergence), except the bivariate surface tail, which `dev/plan-3d-plot.md` now carries.

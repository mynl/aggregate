...
**`Portfolio.pnl_program` cannot be self-contained.** ~~The grammar's `agg_source` admits an inline `agg`, a stored `agg.NAME` or a stored `port.NAME`, and there is no inline portfolio engine, so the portfolio form references (`less port.NAME`) where the aggregate form inlines.~~ **Wrong conclusion, corrected at `a216` as `[Inline-Port-Engine]`.** The grammar was a fact, but treating it as a constraint rather than a gap was the error: the plan's own load-bearing argument for `reins_program`, that text resolving against a knowledge base builds in one session and nowhere else and forces a shared server to write every user's work into one store, applies to a portfolio engine exactly as it does to a cession, and it was not followed through. The author and the `aggregate_api` agent both raised it, on the simpler ground that doing the two engine kinds differently is arbitrary. `agg_source` gained `PORT name as_label agg_list`, the portfolio twin of the inline aggregate engine, reusing the shared `agg_list` minus the trailer. Both engine kinds are now self-contained. The two-statement alternative (the `port` declaration followed by the `pnl`) stays rejected for its own reason: `build` takes exactly one top-level output, so that text would not be something you can hand to `build`.

That correction also removed a lookup that should never have been there. The parser resolved `port.NAME` to its spec at parse time and kept only the name, so the underwriter went back to the store for what the parser already held. Both forms now carry the spec; the underwriter looks nothing up; the referenced name survives only so the writer renders a reference back as a reference.

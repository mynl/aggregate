...
| meta.5 aggregate cleanups + S unification | **done** | 2026-05-30; 3.2 FFT helper extracted to `Aggregate._fft_aggregate` (used by `_freq_sev_convolution`, `update_work` subject hook, and `reinsurance_df`); 3.3 redundant `est_*` writes in `apply_occ_reins` / `apply_agg_reins` deleted; 3.4 `Distortion.price` default flipped backwards → forwards, `DefectiveDistributionWarning(UserWarning)` added to `constants.py` and emitted once per `update_work` when deficit > `VALIDATION_NOISE` (stacklevel=2); 3.6 `aggregate_keys` deleted; 3.7 journey-of-discovery comments scrubbed. Numbers do not move on any baseline case -- the aggregate baseline harness does not call `Distortion.price` (only `Portfolio.price` does, and that already uses forwards via `_build_augmented`). 693 pytest pass; deficit warnings surface on `Def.Pareto`, `Tail.LN`, and `PEG.B` as designed. |
| meta.6 portfolio pricing/allocation | **done** | 2026-05-30; 1.5 `_build_augmented` de-dup with correct L'Hôpital ROE fallback `1/g'(1) − 1` (was `g'(1)` in the efficient branch — TVaR `g'(1)=0` falls through cleanly via `np.inf`); 1.4 `pricing_at` pentagon order `L M P Q a | LR PQ ROE` (added `a` as first-class column); 1.4b `Portfolio.allocation_method` member with cache-invalidating setter, shown in `info`; 1.3 `Aggregate.bounded` + `Portfolio.bounded` (frequency-and-severity inspection, certify-override setter); 1.1 linear default in `Portfolio.price`; 1.2 lifted refused on unbounded+mass (Port.CNC + CCoC raises; Port.Bodoff/Bounded + CCoC still work); 1.7 `exp_loss` and tail-collapse on `exeqa` hoisted out of the per-distortion linear loop. Both baselines regenerated (`tests/baseline/data/` and `tests/data/peg_baseline.json`); 693 pytest pass. |
| meta.7 portfolio cleanup | **done** | 2026-05-30; 3.2 `add_exa_details` slimmed to EPD + reimbursement diagnostic columns only (eta-mu / second-priority surface and `add_eta_mu` deleted; `_build_augmented(efficient=False)` no longer emits `exi_xgtag_ημ_*` / `exag_ημ_*`; `pedagogy.plot_twelve` defensive eta-mu warming removed); 3.3 `swap_density_df` promoted to standalone function in `aggregate.portfolio` (method kept as thin shim) — now refreshes empirical stats via `xsden_to_mwrangler`; 3.5 journey-of-discovery comments scrubbed across `add_exa` and `update` (commented-out alternatives, T.S. Eliot quote, "what is this crap" markers, dead chained-assignment workarounds). `add_exa`'s `ft_nots` argument is now required (the `None` fallback branch was dead). No number movement on baselines; 693 pytest pass. |
| meta.8 parser + perf | **done** | 2026-05-30; 4.1 `so`/`po` disambiguation — both keywords now accept either form (`%` → use directly as share, bare → divide by limit). Parser tracks `%`-suffix via `_PercentNumber` float subclass; arithmetic strips the marker (so `25 * 2` is absolute). Four new corpus cases `J.Re18a..d` cover (so,po) × (percent,bare) and assert identical `(share, limit, attach) = (0.5, 10, 0)`. Existing test_suite.agg unchanged because all PIR cases use the canonical `% so` and `bare po`. 3.5 mixture-arm `gup_sevs` skipped when no exposure row has a positive attachment (saves one Severity per mixture component on the common no-excess path); guard added. 701 pytest pass (was 693; +8 = 4 new lines × parse-and-snapshot tests). No baseline diffs. |

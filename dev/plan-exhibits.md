...
0. **The waterfall gate** (`a207`, awaiting the author's read of the rendered output). Two calls. (a) The **ceded layer rows** carry the risk metrics alongside the gross and net positions. `M / SD` and `M / capital` are natural for a *position* and strained for a *step*: a purchased layer has a negative margin and releases capital in the adverse state, so its diversified capital cell blanks and its `M / SD` reads negative. Options: leave it (current), blank the risk columns on cession rows and evaluate positions only, or add running-net rows so every stop is a position. (b) Whether the walk should also show **amounts ceded** as its own column rather than only the shares, since a waterfall chart is usually drawn from the deltas.
1. **CV as a percent on the ledger** (`a206`, surfaced by rendering). It reads well on the summary card (`0.0%`, `48.8%`, `28.2%`) and badly on a signed ledger, where a negative mean gives readings like `-288.1%`: that is the sign of the mean leaking into a spread measure. Precedent: `Portfolio.validation_df` already switches its spread column from CV to **SD** whenever the portfolio is signed, for exactly this reason, and a P&L ledger is signed by construction. Options: format CV plainly on the ledger and keep the percent on the card; suppress CV from the ledger's insurer view, since SD is already there and is the meaningful spread (RECOMMENDED); or fix it at source in `economic_df`, which is the principled option and matches the portfolio precedent but changes a public frame.
2. **Whether `tail` keeps its insurer treatment** or is demoted to a simple passthrough. RECOMMENDED: keep it. The 1-in-200 and 1-in-250 anchors are exactly the app knowledge this plan exists to migrate.
2. **Per measure formats on the stats exhibit.** ANSWERED IN PART at `a206`. The author's formats (`CV` as `0.0%`, `Skew` as `.3g`) are live as `MEASURE_FORMATS` wherever a measure is a **column**: the summary card and the P&L ledger. They remain impossible on the canonical moment store, where measures run *down* a column and greater_tables declares formats per column. Options there: pivot inside the insurer override, ask greater_tables for a row format concept, or leave the store unformatted. Two follow-ons: `.3g` is not expressible in greater_tables sugar (kinds are `f` / `d` / `%` / `e` / `s`, no `g`), so `Skew` uses `.3f` until GT grows one, which is a cheap upstream ask on a co-developed package; and whether `CV` as a percent reads well for the large CVs a P&L margin can carry is a judgment to make once the ledger is on screen.

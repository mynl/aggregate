...
| `agg` / lee | `p`, Non-exceeding probability, `(0, 1)`, paired with `return_period` (log, `1 .. 1e9`) and `survival`, Exceeding probability, linear or log, `(0, 1)` | `outcome`, shared with the density panel | Aggregate and Severity, role `cdf` | 1-in-100 and 1-in-250, faint | log y, full y, reflect, return period, invert to "Distribution function" |
...
| `pnl` / lee | `p`, paired with `return_period` and `survival` | `outcome`, shared, linear only, full | the same series, role `cdf` | break even (horizontal), 1-in-100 and 1-in-250 faint | full y, reflect, return period, invert |

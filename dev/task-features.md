# Task — maintain `docs/2_aggregate_overview/features.rst`
...
> reconciles `docs/2_aggregate_overview/features.rst` against the current
> development state and adds / updates whatever is missing, then verifies every
> code block executes clean.
>
> **Resume model.** There is no "step N done" — every run starts from the
> coverage ledger inside `features.rst` and the `CHANGELOG.md` feed, computes the
...
> release.
>
> **Ownership and edit flow.** `features.rst` is the **single source of truth
...
> CHANGELOG items, (b) repairs blocks broken by API drift, (c) keeps the ledger
...
> the *content*.
>
> **Format note (since a195).** The page was a Quarto notebook,
> `docs/new_material/features.qmd`, from its first run until the a195 catch-up.
> It is now reStructuredText in the Sphinx tree. The superseded `.qmd` is left
> in `docs/new_material/` for the author to retire; do not maintain both.

---

## 1. Purpose and audience

`features.rst` is the **user-side, topical companion to `CHANGELOG.md`**.

- `CHANGELOG.md` is **developer-facing and chronological** — "what landed, in
  what order, with what breaking-change notes."
- `features.rst` is **user-facing and topical/educational** — "here is what
...
0.30.1** (and, going forward, the `1.0.0b*` series).

**Destiny reached.** The document was written to graduate into the Sphinx docs at
release as a "What changed in 1.0 vs 0.30" chapter, and at a195 it did: it is
`docs/2_aggregate_overview/features.rst`, a real page in the tree, with its code
executed by the `ipython` directive at build time. Keep writing it to that
standard: match the level of detail of the existing docs (the 10-minutes guide
and the feature chapters are the register to aim for), and expect a substantial
document, **tens of pages** when rendered. "Prose is tight" (§2.1) governs
density, not total length; there is a lot of material and it should all be here.

---

## 2. The deliverable: shape of `features.rst`
...
- Code is `.. ipython:: python` blocks, indented four spaces. They **execute at
  ...
  `docs/conf.py`. A figure is captured with a `@savefig <name>.png scale=20`
  line at the block's base indent, immediately before the plotting statement.
- **Runnable source only — no outputs are committed.** The build produces them.
  The task's job is to guarantee every block *runs clean* (§4), not to freeze
  its output.
- Inline code is ``` ``double backticks`` ```; classes referenced as
  `` :class:`Aggregate` ``; other pages as `` :doc:`...` ``. Math is
  `` :math:`...` `` inline and `.. math::` displayed.
...
- **Citations** are `:cite:t:` (narrative) and `:cite:p:` (parenthetical),
  rendered by `sphinxcontrib.bibtex`. Keys are found by searching
  `C:/s/TELOS/Biblio/uber-library.bib`, **never invented**. The page carries no
  references section of its own: `docs/7_bibliography.rst` holds the one
  `.. bibliography::` directive for the whole tree.
...
Section 1 of the page is a **coverage ledger**, a `.. csv-table::` with columns
`Feature`, `Since`, `Section`, `Example object`, mapping every CHANGELOG item to
its treatment in the doc. Each cell is double-quoted, so no cell may contain a
`"`; inline code is `` ``double backticks`` `` as everywhere else:
...
> **The cast's canonical home is `features.rst` §2 itself** (since the first
...
> code. To change the cast, edit `features.rst` §2 directly; there is no
> copy-through step.
>
> **The run has license to edit the cast in `features.rst`** — fix API drift
...
The current roster (see `features.rst` §2 for the live definitions): `simple`,
...
- `uv sync --all-extras`. **Never a single `--extra`**: `uv sync` is an exact
  sync and prunes the extras you did not select, which deletes the `massive` /
  `viz` / `numba` packages and breaks the bivariate suites. `UV_LINK_MODE=copy`
  is set for the harness; in a plain shell `$env:UV_LINK_MODE = "copy"`.
- Run against **`.venv`**, not `.doc-venv`. An ambient
  `UV_PROJECT_ENVIRONMENT=.doc-venv` makes a bare `uv run` resolve to the docs
  environment. Be explicit: `.venv/Scripts/python.exe …`.
...
- Check `git status` / `git diff` for **uncommitted author edits to
  `features.rst`**. They are authoritative — work around them additively, never
...
1. Parse `CHANGELOG.md`: collect every `## 1.0.0aN` (and `b*`) heading and its
   ...
   **First run must read the full file, including `a1`–`a20`**, not only the
   recent entries.
2. Read the coverage ledger (§2.3) from `features.rst` (empty on the first run).
...
Every run ends by **executing every code block headlessly**, in order, in one
namespace, and requiring it to complete with nothing raising:

```
.venv/Scripts/python.exe dev/check_features_rst.py
```

That harness extracts each `.. ipython:: python` block, drops the `@savefig`
directive lines, and `exec`s the rest under the `Agg` matplotlib backend. Exit 0
means every block ran; on the first failure it prints the block number, its
source line in the `.rst`, the source itself, and the traceback. It replaced the
`jupytext --to ipynb --execute` gate the page used while it was a Quarto
notebook: same guarantee, no Jupyter kernel.

**It does not check rendering.** Directive syntax, cross references, the
csv-table and `@savefig` placement are only exercised by a real Sphinx build,
which is deliberately outside this loop (the doc tree is 500+ pages and the
build is slow; see `CLAUDE.md`). Note in the run summary that a rebuild is
pending; the author runs it.

If a block fails, the usual cause is API drift since the example was written —
fix the example to the current API (that *is* part of keeping the doc honest),
or, if the feature itself regressed, stop and report it rather than papering
over it.

**Done when:** every CHANGELOG item has a ledger row; every block executes
clean; the cast is still small and leveraged.
...
- `features.rst` is a doc-only artifact, so updating it **does not itself bump the
...
- Instead, **each `a*` / `b*` release that adds a user-facing feature should run
  this task as part of closing the iteration**, so `features.rst` lands in step
...
- **Catch-up and port, 2026-08-04, at a195.** Gap was 27 releases (a169 to
  a195); the ledger went from 152 rows to **179**, and it now cites a1 to a195
  with nothing uncited. Nine new subsections were added and two existing ones
  repaired. New: §5.1 to §5.3 (inert trailer bodies, the grammar-derived
  colorizer, the canonical library layout), §6.12 to §6.15 (the FCC contract,
  `validation_df` and the narrative pairs, object reprs, exact severity
  quantiles and `oep`), §7.6 (`sharpen`, a192 to a195), §12.4 to §12.6
  (`discover` guards, `interpret_file`, `build` dispatch), §14.5 to §14.7
  (peeling and tier subtotals, `Side`/`Label`, `ratio_df`/`legs_df`), §15.6
  (`axis_support_df` and bivariate labels). **Two repairs, both drift:** §14.3
  said the middle walk step was `ceded occ`, which a183 renamed to the layer's
  own descriptor (`occ 500 xs 500`); and §14.4 described the pre-a187 `evaluate`,
  including a single-obligation-leg restriction that a187 removed, so it was
  rewritten around `rho_g(margin) = 0`. The a169 to a195 surfaces were each
  probed against the live API before being written up. Gate passed: 99 code
  cells, 0 errors.
  The page was then **ported from `.qmd` to `.rst`** and moved to
  `docs/2_aggregate_overview/features.rst`, this spec moved to `dev/`, and the

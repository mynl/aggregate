...
`Link <https://openacttexts.github.io/Loss-Data-Analytics/ChapSeverity.html#MethodsCreation>`_
...
In this case we must use unconditional severity to include the  possibility that the amount paid is 0. This is done by adding ``!`` at the end of the severity specification. The moments are computed exactly without updating.
...
.. ipython:: python
    ...
    m = 1000 * np.exp(-0.1)
    print(a.agg_m, m)
    print(a.agg_var, (2 * 1000**2 * np.exp(-0.1)) - m**2)
...
The trick here is to realize that :math:`X` is a beta variable with :math:`\alpha=2` and :math:`\beta=1`.
...
.. ipython:: python
    ...
    print(a.agg_var)
    with option_context('display.width', 120, 'display.float_format', lambda x: f'{x:.4g}'):
        print(a.describe)
...
.. ipython:: python
    ...
    a.agg_m, a.agg_sd, a.agg_var, a.agg_m + a.agg_sd
...
.. ipython:: python
    ...
    # standard option context
    soc = option_context('display.width', 140, 'display.float_format', lambda x: f'{x:,.5g}')

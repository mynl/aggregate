Probability and Risk Theory (rst)
=================================
...
distribution into a density vector :math:`\x=(x_1,\dots,x_{m})` whose
length :math:`m` is a power of two :math:`m=2^n`. Here

.. math::

    x_i=\text{Pr}((i-1/2)b < X < (i+1/2)b)
    x_1=\text{Pr}(X<b/2),\quad x_{m}=\text{Pr}(X>(m-1/2)b)

for some fixed :math:`b`. We call :math:`b` the bucket size. Note
:math:`\sum_i x_i=1` by construction. The FFT of the :math:`m\times 1` vector
:math:`\x` is another :math:`m\times 1` vector :math:`\hat\x` whose
:math:`j`\ th component is

.. math:: \sum_{k=0}^{2^n-1} x_k\exp(2\pi ijk/2^n).\label{fft}

The coefficients of :math:`\hat\x` are complex numbers. It is also
possible to express :math:`\hat\x=\F\x` where :math:`\F` is an
...
You can use the inverse FFT to recover :math:`\x` from its transform
:math:`\hat\x`. The inverse FFT is computed using the same equation
...
approximation *vector* :math:`\hat\x` and compute

.. math:: \hat\a=M_G(n(\hat\x -1))
...
approximation :math:`\a` of :math:`A` from :math:`\hat\a`! See Wang
...
:math:`\Var(S')=\sigma^{'2}` and let :math:`\sigma^2` be the variance of

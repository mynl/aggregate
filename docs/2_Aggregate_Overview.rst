...
This chapter explains how ``aggregate`` works. It opens with a working session that introduces the DecL language and the main objects, then covers the :class:`Underwriter` and its recipe library, the four computation pipelines (aggregate, reinsurance, portfolio, and P&L), the publishing pipeline that turns any of them into exhibits and charts, the automatic grid sizer, numerical artifacts that look like bugs and are not, the layout of the ``info`` string, what changed in 1.0, and how the library is tested.
...
.. toctree::
   ...
   2_aggregate_overview/pipeline-exhibits-and-charts

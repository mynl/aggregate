...
The same shortfall sets :attr:`~aggregate.constants.Validation.INFEASIBLE` and raises :class:`~aggregate.constants.InfeasibleGridWarning` once per session per severity and grid. The flag is a validation **type** rather than a failure, so it is transparent to ``Validation.passes`` and leads the explanation, explaining the moment flags underneath it rather than adding to them; see :ref:`agg pipeline validation`. Measured over the 255 building programs of the test suite and the shipped library, four set it, ``CurvePareto``, ``GrossCatXOL``, ``HeavyTailValidation`` and ``USXOLTower``, and every one of them was already failing its severity mean without being told why.

...
* flags ``INFEASIBLE`` when the severity cannot be reproduced on the grid at **any** bucket size, the reading described in :ref:`bs feasibility`. This one is a new validation *type* rather than a failure: it is a statement about the grid, so it fires whether or not the moments happen to fail, it leads the explanation the way ``DEFECTIVE`` does because it explains the moment flags underneath it, and it is transparent to ``Validation.passes``. An infeasible object whose moments fail still fails; one whose moments hold still passes, carrying the reading. It deliberately does not join the ``REINSURANCE`` arm, which would let the flag hide the very problem it names.

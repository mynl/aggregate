...
Columns are ``P``, ``L``, ``E``, ``M``, ``LR``, ``ER``, ``CR``, ``E_LR``, ``E_ER``, ``E_CR``, ``P_share`` and ``M_share``. The three amounts are signed in the gross direction: consideration as booked, obligations negated. A cession's ceded premium and recovery are therefore both negative, which makes every ratio built from them come out with its conventional sign, makes the amounts add across blocks, and makes ``M == P - L - E`` hold identically, since it is the signed row sum. Each amount carries its contra: ``L`` absorbs cession recoveries, and since ``1.0.0a304`` ``E`` absorbs ceding commission, the statutory netting against acquisition expense that mirrors recoveries against loss. On a cession block ``ER`` therefore reads as the commission rate and on ``All`` as the expense ratio net of commission; ``legs_df`` keeps every commission leg itemized under its own ``kind``, which is where a split is recovered. ``L``, ``M``, ``P`` and ``LR`` keep the ``pentagon.PENTAGON_STATS`` spelling so a P&L ratio frame concatenates and diffs against a pricing frame; ``E``, ``ER`` and ``CR`` extend it, and ``Q``, ``a``, ``PQ`` and ``ROE`` have no meaning on a ledger.

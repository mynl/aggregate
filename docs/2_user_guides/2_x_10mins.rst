...
The per-distortion result is returned in (and stored on) the ``distortion_df`` dataframe, indexed by distortion. The requested distortions are all single parameter: ``param`` gives the raw shape (``param_name`` names it for each family), ``error`` the miss in achieved premium, ``gini_p`` the comparable normalized shape (the TVaR-equivalent level :math:`2\int g - 1`), and ``area`` the area under the distortion :math:`\int g = (\mathrm{gini\_p}+1)/2`. The shared calibration target is shown once on ``p07.calibration_df`` — the inputs ``coc``, ``p``, ``F(a)`` followed by the pentagon octet ``L, M, P, Q, a, LR, PQ, ROE`` (where ``ROE`` equals the requested ``coc``). The attribute ``p07.distortions`` is a dictionary with keys distortion types and values :class:`Distortion` objects. See PIR REF for more discussion.

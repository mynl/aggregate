.. code-block:: text
    ...
    // `doc{{{...}}}` is a long-form markdown recipe that may contain fenced code.
    // All four are optional and order-free, at most one of each (a repeat is a
    ...
    // exactly one parse -- and it scales: four order-free optional items spelled out
    // would be 65 alternatives. Every item is a distinct token with a disjoint
    // prefix (`note{` / `tags{` / `hints{` / `doc{{{`), so the parse is unambiguous.
    ...
    HINTS.3:        /hints\{[^}]*\}/
    TAGS.3:         /tags\{[^}]*\}/
    // `doc{{{...}}}` never reaches the lexer with its body intact: step 0 of
    // UnderwritingLexer.preprocess lifts the body out and substitutes URL-safe
    // base64, whose alphabet (A-Za-z0-9-_=) contains no `#`, `//`, `}`, `[`, `]`,
    // `;` or whitespace -- so the token survives comment stripping, bracket
    // collapsing and the paragraph split byte-for-byte. That is what lets a doc
    // body carry markdown headings, blank lines and fenced code, none of which
    // could survive inside a `note{...}`. The DOC callback decodes it again.

...
Registered exhibits: `summary`, `stats`, `validation`, `tail`, `reins`, `bs_window`, `tail_behavior`, `economic`, `economic_ratios`, `economic_waterfall`, `dependency`. `available_exhibits(obj)` says which of them an object can serve, which depends on its class and on whether it has been updated.

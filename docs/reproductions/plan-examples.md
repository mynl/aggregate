...
| **Bear1990** | `Bear1990.qmd` | **Reproduced, both reproducible columns.** Table I, all six treaties. The Lognormal Model column comes back at every printed digit by rebuilding the paper's own double linear interpolation in the Appendix B tables; the Collective Risk Model `c=0, b=0` column comes back to the printed two decimals in five of six and within one basis point in the sixth. Appendix F and Appendix J Exhibit 1 reproduce per class, claim counts, excess severity moments and aggregate standard deviations. The `c>0` columns are **not** comparable: they carry a severity parameter uncertainty loading `b` with no DecL analog. Each feature is one clause: `aggregate net of`, `reinstatements 3 free`, `corridor`, `retro`, `pc`, `slide`. |

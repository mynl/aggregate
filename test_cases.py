...
class TestAggregateModule(unittest.TestCase):
    ...
    def test_Aggregate(self):
        ...
        self.assertTrue(d1b1.stats_total.T.loc['el', :].sum() == 5146.408347357843)
        ...
        self.assertTrue(d1b2.stats_total.T.loc['el', :].sum() == 6152.9037739696396)
        ...
        self.assertTrue(d2b1.stats_total.T.loc['el', :].sum() == 11464.315472434911)

        fixed = Aggregate('chistogram', exp_en=1, sev_name='dhistogram', sev_xs=[0, 1, 2, 3, 4], sev_ps=.2,
                          freq_name='fixed')
        self.assertTrue(fixed.statistics_df['sev_cv'].sum() == 2.1213203435596428)
